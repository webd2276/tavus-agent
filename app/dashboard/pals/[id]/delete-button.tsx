...
export function DeleteButton({ palId }: { palId: string }) {
  ...
  return <div className="mt-8"><button onClick={remove} disabled={loading} className="text-sm text-[#E08585] underline disabled:opacity-60">{loading ? "Deleting…" : "Delete PAL"}</button>{error && <p className="mt-2 text-sm text-[#E08585]">{error}</p>}</div>;
}
